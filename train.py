...
class TrainEngine:
    def __init__(self, daily_plot_off, residual_on, use_multistep_learning_rate, scheduler_setting, converge_delta, converge_patience, seed_list, IAO_threshold, result_root_path):
        # hyper-parameters
    ...
    def _transfer_to_index_of_loss_weight_table(self, invm, tau):
        # input : invm = 10, tau = 100
        # ouput : index = invm10.00000_tau100.00000, index.values = array(['invm10.00000_tau100.00000'], dtype=object)
        # 用途是將一組數字創建成一組可供索引的字串
        index = 'invm' + pd.Series(invm).map('{:.5f}'.format).astype(str) + '_tau' + pd.Series(tau).map('{:.5f}'.format).astype(str)
        return index.values
            ...
            for seed_idx, untrained_model in enumerate(modell):
                ...
                for init_lr in self.scdl_setting['init_lr_list']:
                    ...
                    for warmup in self.scdl_setting['epoch_warmup_list']:
                        ...
                        self.model = copy.deepcopy(untrained_model[model_selection])
    ...
    def _train(self):
        ...
        for self.epoch in range(self.scdl_setting['cur_epoch_size']):
            ...
            if ((self.epoch % 10 == 0) & (self.epoch != 0)) or self.epoch == (self.scdl_setting['cur_epoch_size'] - 1):
    ...
    def show_detailed_perfromance(self, dataset, phase, savefig=False):
        ...
        call_time_true = call_true.detach().numpy() - call_intrinsic
        call_time_pred = call_pred.detach().numpy() - call_intrinsic
        ...
        t['call_pred'] = call_pred.detach().numpy()
        t['call_intrinsic'] = call_intrinsic
        t['call_time_true'] = call_time_true
        t['call_time_pred'] = call_time_pred
        t['target_true'] = target_true.detach().numpy()
        t['target_pred'] = target_pred.detach().numpy()
        ...
        plot_3d_relation(tau, invm, {'true': target_true.detach().numpy(), 'pred': target_pred.detach().numpy()}, 'tau', 'invm', f'{phase}_target_value', self.test_date, angle=(30, 0), path=f"{self.validate_folder}/{self.test_date}_{phase}_targetvalue_{self.epoch}epoch.png" if savefig else None)
        print("debug")
    ...
    def _compute_call_price(self, dataset,syn_pred):
        if syn_pred:
